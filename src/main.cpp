...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90; // stake age of full weight


//Version 2.0 will change weight 15/45 days
unsigned int nStakeMinAgeV2 = 60 * 60 * 24 * 1; // minimum age for coin age
...
// 1%(wrong) to 200%
// There will also be a max reward of 100 coins. This will ensure a more constant rate of inflation
...
static const int64 nTargetTimespan = 60;  // 4-hour
static const int64 nTargetSpacingWorkMax = 1.5 * nStakeTargetSpacing; // 2-hour
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    CBitcoinAddress address(!fTestNet ? FOUNDATION_ADDRESS : FOUNDATION_ADDRESS_TEST);
        ...
        if (vtx[0].vout[1].nValue < GetFoundationAmount(nBits))
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
      ...
      else
      {
          ...
          {
               if (vtx[0].GetValueOut() > ((IsProofOfWork()? (GetProofOfWorkReward(nBits) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0) + GetFoundationAmount(nBits)))
                   return DoS(50, error("CheckBlock() : coinbase reward exceeded %s > %s",
                        FormatMoney(vtx[0].GetValueOut()).c_str(),
                        FormatMoney(IsProofOfWork()? GetProofOfWorkReward(nBits) : 0).c_str()));
          }
...
//vMerkleTree: ea6fed5e2
        // Genesis block
        const char* pszTimestamp = !fTestNet ? "03/02/2015-6:33PM AESDST" : "Tranz Testnet";
        CTransaction txNew;
       txNew.nTime = nChainStartTime;
        ...
        if (fTestNet)
           assert(block.hashMerkleRoot == uint256("0x55c7717b0d8765c5b539a0ba0fc1f3398618842a854c47abbd5e06450c97bf6a"));
        else
           assert(block.hashMerkleRoot == uint256("0x55c7717b0d8765c5b539a0ba0fc1f3398618842a854c47abbd5e06450c97bf6a"));
...
unsigned char pchMessageStart[4] = { 0xd8, 0x5a, 0x63, 0x39 };
